...
- **Hobot-VA WatchDog 실시간 IPC 텔레메트리**: VPU 이상 상태를 `/dev/shm/hobot_va_watchdog`로 전송합니다. 임시 파일과 원자적 이름 변경을 사용하여 플레이어가 불완전한 레코드를 읽지 않도록 했습니다.
...
2. **WatchDog IPC 텔레메트리**:
   ...
   ```
   10초간 정상 재생이 지속되면 어노말리 카운터가 자동으로 0으로 초기화됩니다. 플레이어 폴백 기준은 **어노말리 1회**이며, IPC 파일은 원자적으로 갱신되고 컨텍스트 생성/종료 시 정리됩니다.
...
| **CPU 표면 매핑 미지원** | ✅ 완전 해결 | `vaLockSurface`에서 NV12 stride·offset·DMA-BUF 이름·CPU 포인터 제공 |
